...
public class UserController {
     ...
     * 添加管理员
     *
     * @param user 管理员实体，通过@RequestBody接收前端传来的JSON数据
     * @return 操作结果
     */
    @PostMapping("/add")
     ...
     * 更新管理员信息
     *
     * @param user 管理员实体，包含id字段用于标识要更新的记录
     * @return 操作结果
     */
    @PutMapping("/update")
     ...
     * @param user    查询条件（可包含username、name等字段）
     * @return 分页结果
     */
     ...
     * */
    @GetMapping("/export")
    public void exportData(User user, HttpServletResponse httpServletResponse) throws Exception {
//        根据传入的id进行导出
...
//        设置写出的输出流，并关闭httpServletResponse
    ...
    public Result importData(@RequestParam("file") MultipartFile file) throws Exception {
        ...
        return Result.success("成功导入 " + userList.size() + " 条数据");
    }
}