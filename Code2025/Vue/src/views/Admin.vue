...
const deleteAll = () => {
  ...
  ElMessageBox.confirm("你确认删除这些数据吗？（无法恢复）", "删除确认", {
    type: "warning",
      ...
      .then(() => {
        request
            .delete("admin/deleteAll", {data: data.rows})
            .then((res) => {
              if (res.code === "200") {
                ElMessage.success("删除成功");
                load();
...
const exportData = (res) => {
  ...
  let url = `http://localhost:9999/admin/export?`
...
const handleImportSuccess = (res) => {
  let url = `http://localhost:9999/admin/import?token=${data.user.token}`;
  window.open(url);
  if (res.code === '200') {
